...
struct window_t* cpu_lru_translate(uint32_t addr) {
	...
	esp_partition_read(partition_data, win->sector * LRU_SEC_SIZE, win->data, LRU_SEC_SIZE);
	...
	if(tmp->w){
		esp_partition_erase_range(partition_data, tmp->sector * LRU_SEC_SIZE, LRU_SEC_SIZE);
		esp_partition_write(partition_data, tmp->sector * LRU_SEC_SIZE, tmp->data, LRU_SEC_SIZE);
	}
...
void cpu_loop(void *pvParameters){
	...
	for(;;){
		...
		*(uint64_t*) &clint.time = esp_timer_get_time() /*microseconds*/ * 240 /*Mhz*/;
		...
		for(uint16_t x= 28657; x--; /*only to performance, prioritizing the cpu_execute*/){
...
void app_main() {
	...
	partition_data = esp_partition_find_first(ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_ANY, "_memory");

	//
	for(uint32_t x= 0; x < (partition_data->size / LRU_SEC_SIZE); x++) {
	...
	for(uint32_t x= 0; x < (0x800000 /*8MB*/ / LRU_SEC_SIZE); x++) {
		...
		struct window_t *win = &window_[ x + (partition_data->size / LRU_SEC_SIZE) ];
		win->w= 0;
		win->sector= -1;
		win->time= 0x0000;
		win->data= heap_caps_malloc_prefer(LRU_SEC_SIZE, 2, MALLOC_CAP_SPIRAM, MALLOC_CAP_INTERNAL);
...
//				(1 << 11 /*VIRTIO_BLK_F_CONFIG_WCE*/) |
		...
		rootfs.data= fopen("/data/rootfs.ext2", "r");
